...
function AV:ShouldSell(link)
	...
	if itemClassId == Enum.ItemClass.Weapon or itemClassId == Enum.ItemClass.Armor then
		...
		if self.db.profile.soulbound then
			-- sell unusable items
			if itemsBOP[itemId] and not IsEquippableItem(itemId) then
				return true
			end
		end
	end
...
AV_NON_OPTIMAL_ITEMS = {
	['DEATHKNIGHT'] = {
		[Enum.ItemClass.Armor] = { Enum.ItemArmorSubclass.Cloth, Enum.ItemArmorSubclass.Leather, Enum.ItemArmorSubclass.Mail },
	},
	['DEMONHUNTER'] = {
